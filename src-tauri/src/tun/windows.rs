...
#[async_trait]
impl TunManager for WindowsTunManager {
    async fn create_tun(&self, config: TunConfig) -> Result<TunDeviceInfo, String> {
        ...
        crate::log_info!(format!("Windows TUN 已创建: ifname={}, mtu={}", ifname, config.mtu));
    ...
    async fn create_tun_from_fd(&self, _fd: i32, _config: TunConfig) -> Result<TunDeviceInfo, String> {
        Err("Windows 不支持从 fd 创建 TUN".into())
    }
