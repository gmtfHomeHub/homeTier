...
#[async_trait]
impl TunManager for IosTunManager {
    async fn create_tun(&self, _config: TunConfig) -> Result<TunDeviceInfo, String> {
        Err("iOS 上 TUN 设备由 NEPacketTunnelProvider 创建，请使用 create_tun_from_fd".into())
    }
    ...
    async fn create_tun_from_fd(&self, fd: i32, config: TunConfig) -> Result<TunDeviceInfo, String> {
        ...
        crate::log_info!(format!("iOS TUN 已创建: fd={}, mtu={}", fd, config.mtu));
